...
//	cout << "generation "<<endl;
	...
	//this procedure is necesary since the penalized individuals are used with memmory addres
	...
	//sophisticated_Local_Search(child_pop);
...
void MOEA::sophisticated_Local_Search(vector<CIndividual> child_pop)
{
   //Apply initial local search to all the individuals...
//   for(int i = 0 ; i < child_improved.size(); i++) 
//   {
//     global_test(child_improved[i]);//asumption of D x 100 = 3000 nfes
//     step_binary_local_search();
//   }
}
...
double MOEA::mts1_dim(vector<double> &SR, CIndividual &current, int d, CIndividual &bestind)
{
  ...
  nfes++;
    ...
    double score = distance_improvement(current.y_obj, bestind.y_obj) - distance_improvement(bestind.y_obj, current.y_obj);
	//cout << score <<endl;
  if( score > 1e-3  && bestind < current) //checking improvement distance....
    {
    ...
    else if( score < 0.0 )
    {
	 bestind.x_var[d] += 0.5*SR[d];
  	 ...
  	 bestind.x_var[d] = min(bestind.x_var[d],  vuppBound[d]);
  	 bestind.obj_eval();
         score = distance_improvement(current.y_obj, bestind.y_obj) - distance_improvement(bestind.y_obj, current.y_obj);
	 if( score > 1e-3 && bestind < current ) //checking improvement distance....
         {
            improvement_distance = score; 
         }else 
